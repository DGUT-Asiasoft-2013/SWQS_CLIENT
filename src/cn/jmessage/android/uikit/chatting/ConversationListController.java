...
import cn.jmessage.android.uikit.chatting.adapter.ConversationListAdapter;
...
//        }
    ...
    public void onItemClick(AdapterView<?> viewAdapter, View view, int position, long id) {
        ...
        if (position > 0) {
            ...
            intent.putExtra(TradeApplication.CONV_TITLE, conv.getTitle());
            if (null != conv) {
                // 当前点击的会话是否为群组
...
//                    mContext.getActivity().startActivity(intent);
                    ...
                    intent.putExtra(TradeApplication.TARGET_ID, targetId);
                    intent.putExtra(TradeApplication.TARGET_APP_KEY, conv.getTargetAppKey());
                    Log.d("ConversationList", "Target app key from conversation: " + conv.getTargetAppKey());
                    intent.putExtra(TradeApplication.DRAFT, getAdapter().getDraft(conv.getId()));
                }
                intent.setClass(mContext.getActivity(), ChatActivity.class);
                mContext.getActivity().startActivity(intent);
            }
