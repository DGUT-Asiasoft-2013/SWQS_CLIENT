...
//        long groupId = intent.getLongExtra(GROUP_ID, 0);
        long groupId = 0;
        String targetAppKey = intent.getStringExtra(TradeApplication.TARGET_APP_KEY);
        mMessageId = intent.getIntExtra("msgId", 0);
        if (groupId != 0) {
            mConv = JMessageClient.getGroupConversation(groupId);
        } else {
            String targetId = intent.getStringExtra(TradeApplication.TARGET_ID);
            if (targetId != null) {
                mConv = JMessageClient.getSingleConversation(targetId, targetAppKey);
            }
        }
        mStart = intent.getIntExtra("msgCount", 0);
        mPosition = intent.getIntExtra(TradeApplication.POSITION, 0);
...
//                    mLoadBtn.setVisibility(View.VISIBLE);
    ...
    private void initImgPathList() {
        mMsgIdList = this.getIntent().getIntegerArrayListExtra(TradeApplication.MsgIDs);
        Message msg;
        ImageContent ic;
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                case R.id.return_btn:
                    ...
                    setResult(TradeApplication.RESULT_CODE_SELECT_PICTURE, intent);
                    finish();
                    break;
                ...
                imgContent.downloadOriginImage(mMsg, new DownloadCompletionCallback() {
                    ...
                    public void onComplete(int status, String desc, File file) {
                        ...
                        if (status == 0) {
                            ...
                            bundle.putInt(TradeApplication.POSITION, mViewPager.getCurrentItem());
                            msg.setData(bundle);
                            msg.sendToTarget();
                        } else {
                            if (mProgressDialog != null) {
            ...
            if (activity != null) {
                switch (msg.what) {
                    case DOWNLOAD_ORIGIN_IMAGE_SUCCEED:
                        ...
                        activity.mPathList.set(bundle.getInt(TradeApplication.POSITION), bundle.getString("path"));
                    ...
                    case SEND_PICTURE:
                        ...
                        activity.setResult(TradeApplication.RESULT_CODE_BROWSER_PICTURE, intent);
                        activity.finish();
                        break;
                    //显示下载原图进度
